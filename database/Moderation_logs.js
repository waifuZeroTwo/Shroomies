...
module.exports = {
    ...
    // Add this new method to your existing ModerationLogs module
    getBanEndTime: async function(memberId) {
        const collection = db.collection('moderation_logs'); // Replace with your collection name
        const result = await collection.find({
            memberId,
            actionType: 'ban'
        ...
        if (result.length > 0) {
            const record = result[0];
            return record.actionTimestamp.getTime() + (record.duration * 1000);
        }
